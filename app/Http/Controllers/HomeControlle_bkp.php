...
class HomeController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth', ['except' => ['webhookForPayu','webhookForCCAvenue','testJob','GetOrderStatusFromCCAvenue','fixUserMigration']]);
    }
    ...
    public function webhookForPayu(Request $request,$order_id,$status)
    {
        \Log::info("Response from webhook for Payu : ".json_encode($request->all()));
       ...
       // $data = $request->all();
        ...
        $cart = Order::where('id',$data['txnid'])->where('is_payment_attempt','1')->where('user_id',@$user->id)->first();
    ...
    public function webhookForCCAvenue(Request $request)
    {
        ...
        \Log::info("Response from webhook for CCAvenue : ".json_encode($request->all()));
