...
trait CommonHelper
{
  ...
  */
  ...
  public function sendNotification($user,$title,$body,$slug,$data=null,$url=null,$send_by=null){
    ...
    foreach ($devices as $device) {
      if(!empty($device)){
        if(!in_array($device->device_token, $deviceArray)){
          array_push($deviceArray, $device->device_token);
  ...
  */
  ...
  public function sendSMS($user,$mobile_number,$message,$slug='',$country_code='+91'){ 
      if($user == null){
        return true;
      }
    DB::beginTransaction();
    try{
      ...
      $sms->mobile_number  =  $mobile_number;
      $sms->message        =  $message;
      $sms->send_by        =  Setting::get('sms_gateway_sender');
      $sms->status         = 'active';
        ...
        if($business_category_id !=''){
          if($user) {
            $count = Product::whereHas('categories',function($q) use($business_category_id,$childs,$lang)        {
                    $q->whereIn('category_id',$childs);
                  })->where(['business_category_id'=>$business_category_id,'is_live' => '1','status' => 'active','stock_status' => 'in_stock'])->whereIn('visible_to',['dealer','both',$user->user_type])->whereIn('language',$lang)->count();
             }else {
            $count = Product::whereHas('categories',function($q) use($business_category_id,$childs,$lang)        {
                    $q->whereIn('category_id',$childs);
                  })->where(['business_category_id'=>$business_category_id,'is_live' => '1','status' => 'active','stock_status' => 'in_stock'])->whereIn('visible_to',['dealer','retailer','both'])->whereIn('language',$lang)->count();
          }
        }else {
          if($user){
            $count = Product::whereHas('categories',function($q) use($childs,$lang)   {
                    $q->whereIn('category_id',$childs);
                  })->where(['is_live' => '1','status' => 'active','stock_status' => 'in_stock'])->whereIn('visible_to',['dealer','both',$user->user_type])->whereIn('language',$lang)->count();
          }else {
            $count = Product::whereHas('categories',function($q) use($childs,$lang)        {
                    $q->whereIn('category_id',$childs);
                  })->where(['is_live' => '1','status' => 'active'])->whereIn('visible_to',['dealer','retailer','both'])->whereIn('language',$lang)->count();  
          }
        }
       }
       return $count;
