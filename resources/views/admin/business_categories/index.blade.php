    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ trans('business_categories.admin_heading') }}</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('home')}}">{{ trans('common.home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ trans('business_categories.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">{{ trans('business_categories.title') }}</h4>
                ...
                @can('business-category-create')
                  <div class="box-tools pull-right">
                    <a href="{{ route('business_categories.create') }}" class="btn btn-success pull-right">
                    {{ trans('business_categories.add_new') }}
                    </a>
                  </div>
                @endcan
              </div>
                    ...
                    <table class="table zero-configuration data_table_ajax">
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('business_categories.image')}}</th>
                          <th>{{trans('business_categories.name')}}</th>
                          <th>{{trans('business_categories.layout_types')}}</th>
                          <th>Display Order</th>
                          <th>{{trans('common.status')}}</th>
                          <!-- <th>{{trans('business_categories.is_live')}}</th> -->
...
<script type="text/javascript">
  ...
  $(document).on('change','.status',function(){
      ...
      if(confirm){
        ...
        $.ajax({
            type:'post',
            url: "{{route('status_business_categories')}}",
            data: {
                    "status": status, 
                    "id" : id,  
                    "_token": "{{ csrf_token() }}"
   ...
   $(document).on('change','.is_live',function(){
      ...
      if(confirm){
        ...
        $.ajax({
            type:'post',
            url: "{{route('is_live_business_categories')}}",
            data: {
                    "is_live": is_live, 
                    "id" : id,  
                    "_token": "{{ csrf_token() }}"
  ...
  $(document).ready(function(){
    ...
    function fill_datatable() {
      ...
      $('.data_table_ajax').DataTable({ 
        ...
        ajax: {
            url: "{{route('dt_business_categories')}}",
        ...
        columns: [
           ...
           { data: 'layout_type',orderable: false},
           { data: 'display_order',orderable: true},
           { data: 'status',
             ...
             data: 'is_live',
              mRender : function(data, type, row) {
               var is_live=data;
                 ...
                 return '<input name ="publish" class= "is_live" type=checkbox data_id="'+row["id"]+'" '+type+'><label>{{trans("business_categories.publish")}}</label> @can("business-category-edit")  <a class="" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan <a class="" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>';
