                ...
                <div class="col-12">
                    ...
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ trans('order_return.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">{{ trans('order_return.title') }}</h4>
                 @can('order-return-create')
                  <div class="box-tools pull-right">
                    <a href="{{ route('order_return.create') }}" class="btn btn-success pull-right">
                    {{ trans('order_return.add_new') }}
                    </a>
                  </div>
                @endcan
              </div>
                ...
                <div class="card-body card-dashboard">
                  <div class="row">
                    <div class="col-md-6">
                      ...
                      <label></label>
                      <h3><b>Total Refunded Amount : </b>Rs. {{$total_sale_price}}</h3>
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-group">
                      <label></label>
                      <h3><b>Total Return Orders : </b>{{$total_return_orders}}</h3>
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-2">
                      <div class="form-group">
                        ...
                        <select id="order_return_status" class = "form-control" name=''>
                          ...
                          <option value="return_placed">{{trans('order_return.return_placed') }}</option>
                          <option value="dispatched">{{trans('order_return.dispatched') }}</option>
                          <option value="in_review">{{trans('order_return.in_review') }}</option>
                          <option value="accepted">{{trans('order_return.accepted') }}</option>
                          <option value="rejected">{{trans('order_return.rejected') }}</option>
                        </select>
                      </div>
                    </div>
                    <div class="col-md-2">
                      <div class="form-group">
                        <label class="">{{trans('order_return.user_type')}}</label>
                        <select id="user_type" class = "form-control" name=''>
                        <option value=''>{{trans('common.select')}}</option>
                        <option value="dealer">{{trans('order_return.dealer') }}</option>
                        <option value="retailer">{{trans('order_return.retailer') }}</option>
                        </select>
                      </div>
                    </div>
                    <div class="col-md-2">
                    ...
                    <table class="table zero-configuration data_table_ajax">
                      ...
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('order_return.company_name')}}</th>
                          <th>{{trans('order_return.order_return_status')}}</th>
                          <th>{{trans('order_return.number')}}</th>
                          <th>{{trans('order_return.created_date')}}</th>
                          <th>{{trans('order_return.total_price')}}</th>
                          <th>{{trans('order_return.user_type')}}</th>
        ...
        columns: [
           ...
           { data: 'user_type',orderable: false},
           {
            data: '',
                ...
                return '@can("order-return-edit")<a class="" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan';
    ...
    $(document).on('click', '#download_pdf', function(){
        ...
        if(confirm("Are you sure want to download pdf?"))
        {
            //if(id.length > 0) {
                ...
                $.ajax({
                    url:"{{ route('download_order_return_pdf')}}",
                    method:"post",
