    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ trans('profile_titles.admin_heading') }}</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('home')}}">{{ trans('common.home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ trans('profile_titles.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">{{ trans('profile_titles.title') }}</h4>
                @can('profile-titles-create')
                  <div class="box-tools pull-right">
                    <a href="{{ route('profile_titles.create') }}" class="btn btn-success pull-right">
                    {{ trans('profile_titles.add_new') }}
                    </a>
                  </div>
                @endcan
              </div>
                    ...
                    <table class="table zero-configuration data_table_ajax">
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('profile_titles.title')}}</th>
                          <th>{{trans('common.status')}}</th>
                          @if(auth()->user()->can('profile-titles-edit') || auth()->user()->can('profile-titles-list') || user()->can('profile-titles-delete')  )
...
$(document).ready(function(){
  $(document).on('change','.status',function(){
    ...
    if(confirm){
      ...
      $.ajax({
        type:'post',
        url: "{{route('status_profile_titles')}}",
        data: {
                "status": status, 
                "id" : id,  
                "_token": "{{ csrf_token() }}"
    ...
    function fill_datatable() {
      ...
      $('.data_table_ajax').DataTable({
        ...
        language: {
              ...
              emptyTable: "{{trans('datatable.empty_profile_titles')}}",
              zeroRecords:"{{trans('datatable.no_profile_titles')}}",
            },
        ajax: {
            url: "{{route('dt_profile_titles')}}",
        ...
        columns: [
           ...
           { data: 'status',
            ...
            @if(auth()->user()->can('profile-titles-edit') || auth()->user()->can('profile-titles-list') || auth()->user()->can('profile-titles-delete') )
           { 
              mRender : function(data, type, row) {
                return '@can("profile-titles-edit")<a class="btn" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan<a class="btn" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>@csrf</form>@can("profile-titles-delete")<form action="'+row["delete"]+'" method="post"><button class="btn" type="submit" onclick=" return delete_alert()"><i class="fa fa-trash"></i></button>@method("delete")@csrf</form>@endcan';
