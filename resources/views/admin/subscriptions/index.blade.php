    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ trans('subscriptions.admin_heading') }}</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('home')}}">{{ trans('common.home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ trans('subscriptions.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <table class="table zero-configuration data_table_ajax">
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('subscriptions.h_image')}}</th>
                          <th>{{trans('subscriptions.name')}}</th>
                          <th>{{trans('subscriptions.price')}}</th>
                          <th>{{trans('subscriptions.price_per')}}</th>
                          <th>{{trans('subscriptions.validity')}}</th>
                          <th>{{trans('subscriptions.type')}}</th>
                          <th>{{trans('common.status')}}</th>
                          @if(auth()->user()->can('subscriptions-edit') || auth()->user()->can('subscriptions-list') || user()->can('subscriptions-delete')  )
  ...
  $(document).ready(function(){
    ...
    $(document).on('change','.status',function(){
      ...
      if(confirm){
        ...
        $.ajax({
            type:'post',
            url: "{{route('status_subscriptions')}}",
            data: {
                    "status": status, 
                    "id" : id,  
                    "_token": "{{ csrf_token() }}"
    ...
    function fill_datatable() {
      ...
      $('.data_table_ajax').DataTable({
        ...
        language: {
              ...
              emptyTable: "{{trans('datatable.empty_subscriptions')}}",
              zeroRecords:"{{trans('datatable.no_subscriptions')}}",
            },
        ajax: {
            url: "{{route('dt_subscriptions')}}",
        ...
        columns: [
           ...
           { data: 'card_picture',
              mRender : function(data, type, row) {
                  ...
                  return row['card_image'];
                },orderable: false
            },
           { data: 'name',orderable: false},
           { data: 'price',orderable: false},
           { data: 'price_per',orderable: false},
           ...
           @if(auth()->user()->can('subscriptions-edit') || auth()->user()->can('subscriptions-list') || auth()->user()->can('subscriptions-delete') )
            { 
              mRender : function(data, type, row) {
                return '@can("subscriptions-edit")<a class="btn" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan<a class="btn" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>@csrf</form>';
