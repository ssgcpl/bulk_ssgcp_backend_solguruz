    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">{{ trans('reasons.admin_heading') }}</h2>
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('home')}}">{{ trans('common.home') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ trans('reasons.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">{{ trans('reasons.title') }}</h4>
                @can('contact-reason-create')
                  <div class="box-tools pull-right">
                    <a href="{{ route('contact_reasons.create') }}" class="btn btn-success pull-right">
                    ...
                    <table class="table zero-configuration data_table_ajax">
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('reasons.reason_title')}}</th>
                          <th>{{trans('common.status')}}</th>
                          @if(auth()->user()->can('contact-reason-edit') || auth()->user()->can('contact-reason-list') || user()->can('contact-reason-delete')  )
...
$(document).ready(function(){
  $(document).on('change','.status',function(){
    ...
    if(confirm){
      ...
      $.ajax({
        type:'post',
        url: "{{route('status_contact_reason')}}",
        data: {
                "status": status, 
                "id" : id,  
                "_token": "{{ csrf_token() }}"
    ...
    function fill_datatable() {
      ...
      $('.data_table_ajax').DataTable({
        ...
        language: {
              ...
              emptyTable: "{{trans('datatable.empty_contact_reason')}}",
              zeroRecords:"{{trans('datatable.no_contact_reason')}}",
            },
        ajax: {
            url: "{{route('dt_contact_reason')}}",
        ...
        columns: [
           { data: 'id'},
           { data: 'reason_title',orderable: false},
           { data: 'status',
            ...
            @if(auth()->user()->can('contact-reason-edit') || auth()->user()->can('contact-reason-list') || auth()->user()->can('contact-reason-delete') )
           { 
              mRender : function(data, type, row) {
                return '@can("contact-reason-edit")<a class="btn" href="'+row["edit"]+'"><i class="fa fa-edit"></i></a>@endcan<a class="btn" href="'+row["show"]+'"><i class="fa fa-eye"></i></a>@csrf</form>@can("contact-reason-delete")<form action="'+row["delete"]+'" method="post"><button class="btn" type="submit" onclick=" return delete_alert()"><i class="fa fa-trash"></i></button>@method("delete")@csrf</form>@endcan';
