              ...
              <div class="col-12">
                  ...
                  <div class="breadcrumb-wrapper col-12">
                      <ol class="breadcrumb">
                          ...
                          <li class="breadcrumb-item active">{{ trans('order_return.update') }}
                          </li>
                      </ol>
                  </div>
              </div>
            ...
            <div class="card-content">
              <div class="card-body card-dashboard">
                <form method="POST" id="createForm" accept-charset="UTF-8" enctype="multipart/form-data">
                  <input type="hidden" name="order_return_id" id="order_return_id" value="{{$order_return->id}}">
                    @csrf
                     <div class="form-body">
                      <div class="row">
                        <div class="col-md-2">
                          <span><b>Order ID</b> - {{$order_return->id}}</span><br>
                          <span><b>Customer ID</b> - {{$order_return->user_id}}</span>
                        </div>
                        <div class="col-md-4">
                          <span><b>Order Date/Time</b> - {{date("d-m-Y h:i A",strtotime($order_return->returned_at))}}</span>
                        </div>
                        <div class="col-md-4">
                          <div class="form-group">
                            <label class="">{{trans('order_return.order_return_status')}}</label>
                             <select id="order_return_status" class = "form-control" name=''>
                                <option value=''>{{trans('common.select')}}</option>
                                <option value="return_placed" {{ @($order_return->order_status == 'return_placed') ? 'selected':'' }}>{{trans('order_return.return_placed') }}</option>
                                <option value="dispatched" {{ @($order_return->order_status == 'dispatched') ? 'selected':'' }}>{{trans('order_return.dispatched') }}</option>
                                <option value="in_review" {{ @($order_return->order_status == 'in_review') ? 'selected':'' }}>{{trans('order_return.in_review') }}</option>
                                <option value="accepted" {{ @($order_return->order_status == 'accepted') ? 'selected':'' }}>{{trans('order_return.accepted') }}</option>
                                <option value="rejected" {{ @($order_return->order_status == 'rejected') ? 'selected':'' }}>{{trans('order_return.rejected') }}</option>
                              </select>
                          </div>
                        </div>
                        <div class="col-md-2">
                    ...
                    <div class="table-responsive">
                      ...
                      <div class="order_summary"> 
                        ...
                        <div class="row">
                          <div class="col-md-4">
                              <p class="details"><b>Total MRP :</b>  {{$order_return->total_mrp}}<br>
                                <b>Total Sale Price :</b> {{ $order_return->total_sale_price }} <br>
                                <b>Payment via : </b>{{($order_return->payment_type)? ($order_return->payment_type):'Offline'}}<br>
                                <b>Total Accepted Quantity :</b> {{ $order_return->accepted_quantity }}<br>
                                <b>Total Rejected Quantity :</b> {{ $order_return->rejected_quantity }}<br>
                                <b>Total Weight (in KG) : </b>{{number_format($order_return->total_weight,'2','.','.')}}<br>
                               <b>Total Returnable Amount : </b>{{$order_return->accepted_sale_price }}<br>
                              </p>
                          </div>
                          <div class="col-md-4">
                            <div class="form-group">
                           <b>Courier Name</b><br>
                           <!--  <input type="text" class="form-control" id="courier_name" name="courier_name" value="{{$order_return->courier_name}}" readonly> -->
                           @if($order_return->receipt_image != null)
                           <span>{{$order_return->courier_name}}</span>
                           @else 
                           <span>-</span>
                           @endif
                           </div>
                           <div class="form-group">
                            @if($order_return->receipt_image != null)
                           <a href="{{asset($order_return->receipt_image)}}" target="_blank"> <img src="{{asset($order_return->receipt_image)}}" width="30%" height="30%"></a>
                            @endif
                           </div>
                          </div>
                          <div class="col-md-4">
                          <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="tracking_number" name="tracking_number" value="{{$order_return->tracking_number}}" disabled="">
                          </div>
                          </div>
                        </div>
                        <div class="row">
                    ...
                    <div class="row">
                       <div class="col-md-6">
                        ...
                        <div class="col-md-6 note_div">
                          <h4>Note Log</h4>
                            @foreach($order_return_notes as $order_return_note)
                            <p class="details">
                              @if($order_return_note->customer_note !='')
                                <b><u>Customer Note  {{ date('d-m-Y h:i A',strtotime($order_return_note->created_at))}} by {{App\Models\User::find($order_return_note->added_by)->first_name}}</u></b><br>
                                <span>{{ $order_return_note->customer_note }}</span><br>
                                <span>Order No.{{$order_return_note->order_return_id}}  transaction ID  {{@($order_return_note->transaction_id) ? $order_return_note->transaction_id : '-' }} and payment status is  {{@($order_return_note->payment_status) ? $order_return_note->payment_status : '-'}} </span><br><br>
                              @endif
                              @if($order_return_note->admin_note !='')
                                <b><u>Admin Note  {{ date('d-m-Y h:i A',strtotime($order_return_note->created_at))}} by {{App\Models\User::find($order_return_note->added_by)->first_name}}</u></b><br>
                                <span>{{ $order_return_note->admin_note }}</span><br>
                                <span>Order No.{{$order_return_note->order_return_id}}  transaction ID  {{@($order_return_note->transaction_id) ? $order_return_note->transaction_id : '-' }} and payment status is  {{@($order_return_note->payment_status) ? $order_return_note->payment_status : '-'}} </span>
                              @endif
                            </p>
                            @endforeach
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form method="post">
        @csrf
        <div class="modal-body">
          <input type="hidden" class="" id="order_return_id" name="order_return_id" value="{{$order_return->id}}">
          <input type="hidden" class="" id="order_return_user_id" name="order_return_user_id" value="{{$order_return->user_id}}">
  ...
  <script type="text/javascript">
    ...
    function fill_datatable() {
      ...
      $('.data_table_ajax').DataTable({ 
        ...
        ajax: {
            url: "{{route('dt_order_return_items')}}",
            data: {
                    "_token": "{{csrf_token()}}",
        ...
        columns: [
           ...
           { data: 'stock',orderable: false},
           {
            data: '',
                mRender : function(data, type, row) {
                // '+row['verify']+'
                return row['verify']+'<a href="'+row['view_product']+'"><i class="fa fa-eye"></i></a> @can("order-delete")<a class="" href="#" onclick=" return delete_alert('+row["id"]+') "><i class="fa fa-trash"></i><form action="'+row["delete"]+'" method="post" id="form'+row["id"]+'">@method("delete")@csrf</form></a>@endcan';
    ...
    $("#add_return_item").click(function(){
      ...
      var order_return_user_id = $("#order_return_user_id").val();
       ...
       $.ajaxSetup({
        ...
        $.ajax({
                  url: '{{route("add_more_return_item")}}',
    ...
    $(document).on("click","#add_more_return_item_btn",function(){
      ...
      $.ajax({
                  url: '{{route("add_order_return_item")}}',
    ...
    $('#notifyUserForm').submit(function(evt){
          ...
          $.ajax({
                  ...
                  beforeSend: function(xhr){
                      $('#update_return_note').prop('disabled',true);
                      $('#update_return_note').val("{{trans('common.updating')}}");
                    },
                  success: function(response) {
                      console.log(response);
                      if(response.success) {
                      ...
                      $('#update_return_note').prop('disabled',false);
                      $('#update_return_note').val("{{trans('common.update')}}");
                  }
          });
    }) 
    ...
    if (event.keyCode === 13) { 
        ...
        var count = $("#total_count").val();
          ...
          $.ajax({
                  url: '{{route("verify_product_barcodes")}}',
