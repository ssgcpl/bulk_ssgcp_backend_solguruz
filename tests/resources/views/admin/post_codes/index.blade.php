                ...
                <div class="col-12">
                    ...
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ trans('post_codes.plural') }}
                            </li>
                        </ol>
                    </div>
                </div>
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title">{{ trans('post_codes.title') }}</h4>
                @can('post_code-create')
                  <div class="box-tools pull-right">
                    <a href="{{ route('post_codes.create') }}" class="btn btn-success pull-right">
                    {{ trans('post_codes.add_new') }}
                    </a>
                  </div>
                @endcan
              </div>
                    ...
                    <table class="table zero-configuration data_table_ajax">            
                      <thead>
                        <tr>
                          ...
                          <th>{{trans('post_codes.postal_code')}}</th>
                          <th>{{trans('post_codes.city_name')}}</th>
                          <th>{{trans('post_codes.state_name')}}</th>
                          <th>{{trans('post_codes.country_name')}}</th>
  ...
  $(document).ready(function(){
    ...
    function fill_datatable() {
      $('.data_table_ajax').DataTable({ 
        ...
        ajax: {
            url: "{{route('dt_post_codes')}}",
            data: {"_token": "{{csrf_token()}}"},
        },
        columns: [
             { data: 'id'},
             { data: 'postcode',orderable: false},
